// Last updated: 11/17/2025, 4:37:13 PM
type Point = {
  dis: number;
  point: number[];
};
...
function kClosest(points: number[][], k: number): number[][] {
  const heap = new GenericHeap<Point>((a, b) => b.dis - a.dis);

  for (let point of points) {
    heap.insert({ point, dis: getDistance(point) });
  ...
  return heap.data.map((o) => o.point);
}
